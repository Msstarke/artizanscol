...
- Fixed authenticated save requests failing with `Failed to fetch` by adding an explicit unauthenticated `OPTIONS /v1/{proxy+}` route for API Gateway preflight requests and a matching post-deploy smoke test for `/v1/me/profile` CORS preflight behavior.
